...
def get_writer_agent(project_name: str = "general", persona: Optional[Dict[str, Any]] = None) -> Agent:
    ...
    instructions = [
        ...
        "If you are requested to compile an Astrology or Tarot report, YOU MUST strictly enforce the following 4-node architecture and synthesis rules:",
        "1. Executive Summary: High-level overview of the day's energy. Address the user directly as 'You'.",
        "2. Astrology Diagnostic: Technical breakdown of transits and aspects. Ensure technical depth (degrees, houses).",
        ...
        "CRITICAL: YOUR ROLE IS A SYNTHESIZER. Address the user directly ('You', 'Your') throughout.",
        "GROUND TRUTH: Treat the data from previous steps (Researcher, TarotAgent) as the absolute ground truth.",
        "SINGLE OUTPUT: Produce ONE consolidated report. Do NOT repeat the date multiple times. Do NOT create multiple headers for the same report.",
        "NO HALLUCINATIONS: Do not invent card draws or astronomical data not found in the context.",
        "IMPORTANT: When instructed to output to Google Docs, use the `post_to_google_doc` tool after generating your response ONE TIME with the final consolidated content.",
        "--- END REPORTING RULES ---"
    ]
